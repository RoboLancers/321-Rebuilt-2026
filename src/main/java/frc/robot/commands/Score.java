...
public class Score {
  ...
  public static Command shootFuelWithVelocity(Shooter shooter, Indexer indexer, Tunnel tunnel, Command command){
    return Index.index(indexer).alongWith(new PassFuel(tunnel)).alongWith(command);
  }
  ...
  public static Command scoreFuelFromAnywhere(
    ...
    return Align.rotateToHub(drivetrain, hubHeading, robotPose)
        .andThen(HoodCommands.goToAngle(hood, ()->getScoreAngle(robotPose)))
        .alongWith(Index.index(indexer).alongWith(new PassFuel(tunnel)).alongWith(ShootFuel.outtakeWithVelocity(shooter, ()->getScoreVelocity(robotPose))));
  }

  public static Command shootFuelFromAnywhere(
      Drivetrain drivetrain, Shooter shooter, Hood hood, Supplier<Pose2d> robotPose) {
    return 
        HoodCommands.goToAngle(hood, () -> getScoreAngle(robotPose)).andThen(ShootFuel.outtakeWithVelocity(shooter,()->getScoreVelocity(robotPose)));
  }
