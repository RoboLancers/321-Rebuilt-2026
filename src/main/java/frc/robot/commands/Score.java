...
public class Score {
  ...
  public static Command scoreFuelFromAnywhere(Drivetrain drivetrain, Outtake outtake, Hood hood, Supplier<Rotation2d> hubHeading, Supplier<Pose2d> robotPose) {
    return Align.rotateToHub(drivetrain, hubHeading, robotPose)
        .andThen(
            OuttakeFuel.outtakeWithVelocity(
                outtake, () -> getScoreVelocity(robotPose)))
        .alongWith(
            HoodCommands.goToAngle(
                hood, () -> getScoreAngle(robotPose)));
  }

  public static Command shootFuelFromAnywhere(Drivetrain drivetrain, Outtake outtake, Hood hood, Supplier<Pose2d> robotPose) {
  ...
  public static Command scoreFuelWhileDriving(
      ...
      Outtake outtake,
      Hood hood, Supplier<Rotation2d> hubHeading, Supplier<Pose2d> robotPose) {
    return Align.rotateToHubWhileDriving(drivetrain, translationX, translationY, hubHeading, robotPose)
